...
class TestChirp(unittest.TestCase):
    def setUp(self):
        try:
            ...
            self.truth_lin_2 = np.load(os.path.join('signals', 'chirpsig_lin_2.npy'))
            self.truth_lin_2_arrange = np.load(os.path.join('signals', 'chirpsig_lin_2_arrange.npy'))

            # load exp chirp with params createChirpSignal(200, 1, 1, 20, False) with arrange and linspace as x-axis
            self.truth_exp_2 = np.load('../../exercise1/signals/chirpsig_EX_2.npy')
            self.truth_exp_2_arrange = np.load('../../exercise1/signals/chirpsig_EX_2.npy')
            '../../exercise1/signals/chirpsig_lin_2.npy'
